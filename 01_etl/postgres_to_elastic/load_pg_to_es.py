...
from common.models.models import GenresPG, MoviesPG, PersonsPG
...
# migrate control postfixes
MIGRATE_CONTROL_POSTFIXES = ('genres', 'movies', 'persons')

# validators assotioations
MODEL_VALIDATORS = {
    'genres': GenresPG,
    'movies': MoviesPG,
    'persons': PersonsPG,
        ...
        for status_ok, model_updated in helpers.streaming_bulk(es, query_es,
            ...
            if status_ok:
                ...
                redis.data_handle('hmset',
                                  'migrate_control_{0}'.format(postfix),
                                  redis_dict_process,
                                  **dsl_rs)
...
if __name__ == '__main__':
    ...
    for postfix in MIGRATE_CONTROL_POSTFIXES:
        redis_dict_init = {'id': uuid_initial, 'updated_at': dt_initial}
        redis.data_handle('hmset', 'migrate_control_{0}'.format(postfix),
                          redis_dict_init, **dsl_rs)
    ...
    while True:
        ...
        for postfix in MIGRATE_CONTROL_POSTFIXES:
            query = cfp.get(postfix, "select", raw=True).replace('\n', ' ')
            query_pg_variables = redis.data_handle('hgetall',
                                                   'migrate_control_{0}'
                                                   .format(postfix),
                                                   **dsl_rs)
            ...
            validator_model = MODEL_VALIDATORS.get(postfix)
            valid = (validator_model.parse_obj(item) for item in step_1)

            # es query constructor section
            query = [{'_index': postfix, '_id': data.id, '_source': data.json()}
                     for
                     data in valid]

            # es migrate section
            migrate_pg_es(query, redis, dsl_es, dsl_rs, postfix=postfix,
                          chunk_size=chunk_size)
